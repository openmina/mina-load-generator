...
function parseTransaction(tx: GraphqlZkappCommand): ZkappCommand {
  let failure =
    tx.failureReason === null || tx.failureReason === undefined
      ? undefined
      : {
...
export interface TransactionsAccess {
  waitAll(txs: TransactionId[]): Promise<void>;
}

export class LocalChainTransactions implements TransactionsAccess {
  waitAll(_txs: TransactionId[]): Promise<void> {
    return Promise.resolve();
  }
}

export class BlockchainTransactions implements TransactionsAccess {
  mina: MinaConnection & MinaGraphQL;
  lastBlockHeight: number | undefined;

  log: Logger<any>;

  constructor(mina: MinaConnection & MinaGraphQL) {
    this.mina = mina;
  ...
  async #loadBestChain(length: number): Promise<Block[]> {
    ...
    let [resp, error] = await makeGraphqlRequest(
      lastBlocksQueryFailureCheck(length),
      this.mina
    );
  ...
  async waitAll(txs: TransactionId[]): Promise<void> {
    let txSet = new Set(txs.map((tx) => tx.hash()));
    this.log.info(`waiting for ${txSet.size} transactions`);
    while (txSet.size > 0) {
      let blocks = await this.#fetchNewBlocks();
      for (const block of blocks) {
        this.log.debug(
          `looking for transactions in state ${block.hash} at height ${block.height}`
        );
        for (const zkappCommand of block.txs) {
          if (txSet.delete(zkappCommand.hash)) {
            this.log.info(
              `tx ${zkappCommand.hash} is included in block ${block.hash} at height ${block.height}`
            );
          }
        }
      }
